...
- **忽略单次超时**：设置面板「常规」页开关，开启后孤立的超时样本不显示也不计入统计，仅连续两次及以上的超时保留
- **右键菜单**：编辑、启用/禁用、删除
...
- **新拟态质感**（Neumorphism / Soft UI）：VS Code 式浮动工作台，顶栏与侧栏融为深色 chrome，详情壳作为内容面浮出于细缝；三层表面按明度分层（chrome 最深、凹陷井居中、内容面最亮），元素立体感由左上高光 + 右下暗影双向阴影塑造
- **一键切换**：基于 CSS 变量动态切换，无需刷新；图表柱色、坐标轴、网格逐主题单独设计
- **自定义颜色**：可分别设置别名与 IP 文本颜色
- **外观跟随应用**：主题等外观配置保存在应用本身（而非数据文件），启动时开始页即呈现上次主题；窗口标题栏深浅色同步跟随主题，Windows 11 上标题栏/文字/边框直接采用主题表面色（Windows 10 仅深浅两档，系统限制）
