...
- 视觉体系为新拟态（Soft UI）：VS Code 式浮动工作台 + 三层同色相不透明表面按明度分层——窗体缝隙底色（`panelBackground`）最深，自 `--seam`（6px）细缝露出；顶栏与三块工作台面板（`background`）为圆角浮动面，面板层不投影；详情壳/卡片/按钮等内容面（`cardBackground`）最亮。元素层立体感由 `shadowLight`/`shadowDark` 双向阴影塑造（凸起 raised / 凹陷 inset）；禁止半透明表面、背景模糊与硬边框装饰（唯一例外：弹窗遮罩 `.modalOverlay`）。新增主题时须保持分层关系：`shadowLight` 亮于 `cardBackground`、`shadowDark` 深于 `panelBackground`，文字对三层表面 ≥ 4.5:1。
